...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game){
  ...
  Player 		*player = 		NULL;
  Frame_color	color_player =  BLUE;
  CommandCode 	last_cmd = 		UNKNOWN;
  Command*		last_cmd_point = NULL;
  ...
  if(!player) return;
  last_cmd_point = game_get_last_command(game);
  if(!last_cmd){ free(player); return;}

  last_cmd = command_get_code(last_cmd_point);
  color_player = BLUE + game_get_turn(game);
